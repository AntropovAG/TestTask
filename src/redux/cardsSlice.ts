...
>("cards/fetchCards", async (_, { rejectWithValue }) => {
    try {
        const response = await fetch(
            "https://boringapi.com/api/v1/photos/?page=1&limit=10"
        );
        if (!response.ok) {
    ...
    reducers: {
        ...
        setLike: (state, action: PayloadAction<number>) => {
            ...
            if (state.filter === "liked") {
                state.displayedCards = state.cards.filter(
...
export const { deleteCard, setLike, filterCards, createCard, setFilter } =
    cardsSlice.actions;
